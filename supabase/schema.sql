...
-- ═══════════════════════════════════════════════════════════════════════
-- SCHEMA ADDITIONS — Run these if you already ran the base schema
-- ═══════════════════════════════════════════════════════════════════════

-- Multi-product support for Sales
...
-- Multi-item support for Purchases
...
-- Employee → Auth User linking (Phase 6 identity fix)
...
CREATE TABLE IF NOT EXISTS investments (
  ...
  investment_type   TEXT DEFAULT 'loan',
  amount            NUMERIC(14,2) DEFAULT 0,
  expected_return   NUMERIC(14,2) DEFAULT 0,
  return_percentage NUMERIC(6,2)  DEFAULT 0,
  returned_amount   NUMERIC(14,2) DEFAULT 0,
  profit            NUMERIC(14,2) DEFAULT 0,
  loss              NUMERIC(14,2) DEFAULT 0,
  duration          TEXT,
  status            TEXT DEFAULT 'active'
                      CHECK (status IN ('active','returned','partial','pending','lost')),
...
CREATE POLICY "auth_all" ON investments FOR ALL USING (auth.role() = 'authenticated');
