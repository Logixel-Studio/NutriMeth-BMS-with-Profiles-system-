...
export default function InvoiceExpandedRow({
  row, onEdit, onDelete, canUpdate, canDelete: canDel,
  canExport, canPrint, companySettings
}) {
  ...
  const countMutation = useMutation({
    mutationFn: ({ field }) =>
      base44.entities.Invoice.update(row.id, { [field]: (row[field] || 0) + 1 }),
  ...
  const handleDownload = async () => {
    ...
    downloadInvoicePDF(row, companySettings);
    ...
    <div className="space-y-4 text-sm">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        <div>
          ...
          <p className="font-mono font-bold text-primary">{row.invoice_number}</p>
        </div>
        <div>
          <p className="text-xs text-muted-foreground">Type</p>
          <Badge variant="outline" className={row.invoice_type === 'client'
        ...
        <div>
          <p className="text-xs text-muted-foreground">{row.invoice_type === 'client' ? 'Client' : 'Supplier'}</p>
        ...
        {row.party_email && <div><p className="text-xs text-muted-foreground">Email</p><p>{row.party_email}</p></div>}
        ...
        <div>
          ...
          <p className={cn(isOverdue ? 'text-red-600 font-medium' : '')}>
            ...
            {days !== null && row.payment_status !== 'paid' && (
            ...
            <tbody className="divide-y divide-border/50">
              {row.items.map((it, i) => (
                <tr key={i} className="hover:bg-muted/20">
                  ...
                  <td className="py-2.5 px-3 text-right">{formatCurrency(it.unit_price)}</td>
                  <td className="py-2.5 px-3 text-right font-semibold">{formatCurrency(it.total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        {row.tax_percent > 0 && <div><p className="text-xs text-muted-foreground">Tax ({row.tax_percent}%)</p><p>+{formatCurrency(row.tax_amount)}</p></div>}
        {row.discount_percent > 0 && <div><p className="text-xs text-muted-foreground">Discount ({row.discount_percent}%)</p><p>-{formatCurrency(row.discount_amount)}</p></div>}
        ...
        {row.iban && <div><p className="text-xs text-muted-foreground">IBAN / Virtual</p><p className="font-mono text-xs">{row.iban}</p></div>}
        <div>
          <p className="text-xs text-muted-foreground">Downloads / Prints</p>
          <p>{row.download_count || 0} / {row.print_count || 0}</p>
        </div>
      </div>

      {row.notes && <div><p className="text-xs text-muted-foreground">Notes</p><p>{row.notes}</p></div>}
      {row.terms && <div><p className="text-xs text-muted-foreground">Terms</p><p className="text-muted-foreground">{row.terms}</p></div>}
      ...
      <div className="flex gap-2 flex-wrap pt-2 border-t border-border">
        ...
        {(canPrint !== false) && (
          <Button variant="outline" size="sm" className="gap-1.5"
            ...
            {row.print_count > 0 ? 'Re-Print' : 'Print'}
          </Button>
        )}
