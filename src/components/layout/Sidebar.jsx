...
import {
  ...
  Shield, Building2, Clock, FileText, DollarSign,
  BarChart3, Target, UserCheck, Briefcase,
  GanttChartSquare, AlertTriangle, Activity, Database,
  Globe, Lock, Server, PieChart, CalendarDays, Award,
  CheckSquare, Navigation, LogOut
} from 'lucide-react';
...
const NAV = {
  super_admin: {
    ...
    bottom: [{ path: '/settings', label: 'Settings', icon: Settings }, { path: '/profile', label: 'Profile', icon: UserCircle }]
  },
  admin: {
    main: [
    ...
    bottom: [{ path: '/team', label: 'Team', icon: UsersRound }, { path: '/settings', label: 'Settings', icon: Settings }, { path: '/profile', label: 'Profile', icon: UserCircle }]
  },
  manager: {
    main: [
  ...
  hr: {
    main: [
      ...
      { path: '/hr/salary',         label: 'Salary Setup',   icon: CreditCard },
  ...
  accountant: {
    main: [
      ...
      { path: '/accountant/reports', label: 'Reports',            icon: BarChart3 },
    ],
    bottom: [{ path: '/profile', label: 'Profile', icon: UserCircle }]
  },
  inventory_manager: {
    main: [
  ...
  employee: {
    main: [
      ...
      { path: '/employee/salary',      label: 'Salary & Payslips', icon: DollarSign },
...
export default function Sidebar({ collapsed, setCollapsed, isMobile = false, onClose }) {
  ...
  const nav = NAV[userRole] || NAV['employee'];
    ...
    <motion.aside
      ...
      <div className="py-2 px-2 space-y-0.5 border-t border-sidebar-border flex-shrink-0">
        ...
        <motion.button whileHover={{ x: 2 }} onClick={logout}
          className={cn('w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 group text-sidebar-foreground hover:bg-red-500/10 hover:text-red-400', collapsed && !isMobile ? 'justify-center' : '')}
          title={collapsed && !isMobile ? 'Logout' : undefined}>
          <LogOut className="w-5 h-5 flex-shrink-0" />
          <AnimatePresence>
            {(!collapsed || isMobile) && (
              <motion.span initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }}
                className="text-sm font-medium whitespace-nowrap overflow-hidden">Logout</motion.span>
            )}
          </AnimatePresence>
        </motion.button>
      </div>
