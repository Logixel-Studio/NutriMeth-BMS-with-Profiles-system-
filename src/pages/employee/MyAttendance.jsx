...
import { MapPin, Clock, Wifi, WifiOff, AlertCircle, Activity, Zap } from 'lucide-react';
...
export default function MyAttendance() {
  ...
  // ── Phase 6 fix: single source of truth for employee identity ──
  ...
  const allowedLocation = position && allowedOfficeLocations.length > 0
    ...
    : allowedOfficeLocations.length === 0 && allOfficeLocations.length === 0
      ? { noRestriction: true } : null;
  ...
  const getRadiusMessage = () => {
    ...
    return { ok: false, msg: '❌ You are outside all assigned office locations.' };
  };
  ...
  const checkInMutation = useMutation({
    mutationFn: async () => {
      if (!empId) throw new Error('Employee record not found. Ask admin to link your email.');
      const now      = new Date();
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['my-attendance', empId] }); toast.success('Checked in!'); },
  ...
  const checkOutMutation = useMutation({
    mutationFn: (record) => {
      ...
      return base44.entities.Attendance.update(record.id, {
        ...
        working_hours:  calcWorkingHours(record.check_in, timeStr).toFixed(2),
  ...
  const monthAtt = attendance.filter(a => a.date?.startsWith(today.slice(0, 7)));
  ...
  return (
    <div>
      <PageHeader title="My Attendance" description="GPS-verified attendance records" />

      {/* Employee not linked warning */}
      {!empId && user && (
        <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-700">
          ⚠️ Your account is not linked to an employee record. Ask your admin to add your email
          <strong> {user.email}</strong> to your employee profile.
        </div>
      )}
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            {position
              ? <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200 text-xs"><Wifi className="w-3 h-3 mr-1" />GPS Ready</Badge>
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          {[
            { label: 'Check In', val: todayAtt?.check_in, color: 'text-emerald-600', gps: todayAtt?.check_in_lat },
            { label: 'Check Out', val: todayAtt?.check_out, color: '', gps: todayAtt?.check_out_lat },
            { label: 'Working Hours', val: todayAtt?.working_hours ? `${Number(todayAtt.working_hours).toFixed(1)}h` : null, color: 'text-primary', gps: null },
          ].map(({ label, val, color, gps }) => (
            <div key={label} className="text-center p-4 bg-card rounded-xl border border-border">
              ...
              {gps && <p className="text-xs text-emerald-600 mt-1">📍 GPS Verified</p>}
              {label === 'Working Hours' && todayAtt?.status && (
                <p className={cn('text-xs mt-1 capitalize font-medium', todayAtt.status === 'late' ? 'text-amber-600' : 'text-emerald-600')}>
                  {todayAtt.status}
                </p>
              )}
            </div>
        ...
        {radiusStatus && (
          <div className={cn('text-xs mb-3 px-3 py-2 rounded-lg flex items-center gap-2',
            radiusStatus.ok ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-red-50 text-red-700 border border-red-200')}>
            <MapPin className="w-3 h-3 flex-shrink-0" />{radiusStatus.msg}
          </div>
        )}
        ...
        {assignments.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {assignments.map(a => (
              <Badge key={a.id} variant="outline" className="text-xs bg-card"><MapPin className="w-2.5 h-2.5 mr-1" />{a.office_location_name}</Badge>
            ))}
          </div>
        )}
        ...
        <div className="flex gap-3">
          {!todayAtt?.check_in && (
            <Button className="flex-1 bg-emerald-600 hover:bg-emerald-700 h-11"
              disabled={!position || !isInsideRadius || checkInMutation.isPending || !empId}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left">
                {['Date','Check In','Check Out','Hours','OT','Late','Status','GPS'].map(h => (
            ...
            <tbody>
              ...
              {!isLoading && attendance.length === 0 && <tr><td colSpan={8} className="py-8 text-center text-muted-foreground">No attendance records</td></tr>}
              {attendance.map(a => (
                <tr key={a.id} className="border-b border-border/40 last:border-0 hover:bg-muted/20">
                  ...
                  <td className="py-2.5"><Badge variant="outline" className={cn('text-xs capitalize', statusStyle[a.status] || '')}>{a.status?.replace('_',' ')}</Badge></td>
