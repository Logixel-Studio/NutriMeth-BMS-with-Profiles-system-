...
export default function MyTasks() {
  ...
  // Fetch by email as fallback (tasks created before ID was stored)
    ...
    <div>
      ...
      {!empId && user && (
        <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-700">
          ⚠️ Your account is not linked to an employee record. Tasks assigned to <strong>{user.email}</strong> will still appear.
        </div>
      )}
        ...
        <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    {t.due_date && (
                      <span className={cn('text-xs flex items-center gap-1',
                        new Date(t.due_date) < new Date() && t.status !== 'done' ? 'text-red-600' : 'text-muted-foreground')}>
          ...
          {!isLoading && filtered.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-10">
              {filter === 'done' ? 'No completed tasks yet' : 'No active tasks assigned to you'}
            </p>
          )}
        </div>
      </motion.div>
